# Hi, I'm Cong  👋
### Full-Stack Software Developer | Open Source Enthusiast

I am a self-taught developer based in **Toronto** with a passion for building software that solves real-world problems. With a background in Computer Science and 5+ years of professional experience, I specialize in crafting robust applications using modern web technologies and exploring low-level systems.

---

### 🚀 Technical Skills

| Category | Tools & Technologies |
| :--- | :--- |
| **Frontend** | React, TypeScript, JavaScript (ES6+), Tailwind CSS, SCSS |
| **Backend** | Node.js, Express, Go, Python, Java |
| **Database** | MongoDB, PostgreSQL, MS SQL, Prisma |
| **Systems/Other** | C/C++, Docker, DevOps, Vim, Git, Kubernetes, Railway, AWS, Supabase |

---

### 🛠️ Featured Projects
### [gsheetbase.com](https://github.com/congdv/gsheetbase.com)
- **Tech:** TypeScript, Go, Gin, Postgres, Railway, Docker
- **Focus:** Deliver REST API from google sheet

#### [30 Days of Sharing](https://github.com/congdv/30daysofsharing)
*A personal initiative to document and share 30 days of technical learning and insights.*
- **Tech:** JavaScript, Next.js
- **Focus:** Community engagement and knowledge sharing.

### [Feedback App](https://github.com/congdv/feedback)
- **Tech:** Next.js, Prisma, Postgres, 
- **Focus:** Multiple tenant Sass app

#### [Ghi](https://github.com/congdv/ghi)
*A lightweight text editor built from scratch in C.*
- **Tech:** C
- **Learning:** Terminal I/O, memory management, and low-level system calls.

#### [Snake Clone](https://github.com/congdv/snake-clone)
*A retro-inspired Snake game with a modern touch.*
- **Demo:** [snake.congdv.com](https://snake.congdv.com/)
- **Tech:** JavaScript, HTML5 Canvas

#### [NoteMD](https://github.com/congdv/notemd.app)
*A clean, efficient markdown note-taking application.*
- **Tech:** TypeScript

### [Photo Notes - Mobile App](https://github.com/congdv/photo-notes)
- **Tech:** React Native
- **Focus:** Deliver Mobile App
---

### 📫 Connect with Me
- **Portfolio:** [congdv.com](https://congdv.com)
- **Twitter:** [@congdao9](https://twitter.com/congdao9)
- **Email:** congdaovan94[at]gmail{.}com
- **Location:** Toronto, Canada 🇨🇦
